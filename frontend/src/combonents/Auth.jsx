...
const Auth = ({ onLogin }) => {
  ...
  const handleDemoGoogleLogin = async () => {
    ...
    try {
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
      const response = await fetch(`${apiBaseUrl}/google-auth`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      setError('Google authentication error. Please try email/password login.');
    }
  };
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";
      const authResponse = await fetch(`${apiBaseUrl}/google-auth`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
